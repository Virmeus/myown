...
// POST /api/auth/register - First-time registration
  ...
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ 
        success: false, 
        error: 'Администратор уже зарегистрирован. Используйте вход.' 
      });
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!validPassword) {
      ...
      return res.status(401).json({ 
        success: false, 
        error: `Неверный пароль. Осталось попыток: ${5 - failedAttempts}` 
      });
    }
...
router.get('/sessions', authenticateToken, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Ошибка получения сессий' });
  }
});
...
router.delete('/sessions/:id', authenticateToken, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Ошибка отзыва сессии' });
  }
});

// GET /api/auth/check - Check if registration needed
